...
while len(M_h) >= 2 and len(c_1_2) >= 1: # Step 4
    ...
    schedule_combined_job(mh2, class2combined[cm][1], True)
...
while len(c_1_2) >= 2: # Step 2
    ...
    for job in class2combined[c1]:
        schedule_combined_job(M, job, True)
    for job in class2combined[c2]:
        schedule_combined_job(M, job, False)
